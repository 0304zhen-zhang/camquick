...
//					},
	...
	function pullupRefresh() {
		setTimeout(function() {
			...
			for (var i = cells.length, len = i + 20; i < len; i++) {
				var li = document.createElement('li');
				li.className = 'mui-table-view-cell';
				li.innerHTML = '<li data-id="'+i+'" class="animated bounceIn pp" ><p class="img" data-image="img/'+i+'.large.jpg" style="background:url(img/'+i+'.jpg?1) no-repeat 50% 30%;"></p></li>';
	...
	mui('.mui-scroll').on('tap','.animated',function(){
		var src = $(this).find('p').attr('data-image');
		$(".large").css('display','block').find('img').attr('src',src);
	});
